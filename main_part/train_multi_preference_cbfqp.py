...
from cbf_qp_train_layer import CBFQPTrainConfig, CBFQPProjectorNGT
...
class MultiPreferenceConfig(BaseConfig):
    ...
    def __init__(self):
        ...
        # Distillation warmup: only apply after this epoch (let model learn basic mapping first)
        self.multi_pref_cbf_distill_warmup_epoch = int(os.environ.get('MULTI_PREF_CBF_DISTILL_WARMUP', '200'))
        # Distillation threshold: only apply if projection changed delta by more than this
        self.multi_pref_cbf_distill_eps = float(os.environ.get('MULTI_PREF_CBF_DISTILL_EPS', '1e-4'))
...
def _train_trajectory_step(
    ...
    Loss design ("two-leg" approach when CBF-QP is enabled):
    - loss_v_raw: MSE(v_pred, v_target) - main supervision on raw velocity (preserves learning target)
    - loss_endpoint: smooth_l1(x_pred - x_next) - uses projected trajectory (ensures actual execution aligns)
    - loss_distill: MSE(v_pred, v_used.detach()) - only when projection changed, with warmup
    
    Total: L = alpha * loss_v_raw + beta * loss_endpoint + gamma * loss_distill
    """
    B = batch_x.shape[0]
    ...
    # [CBF-QP TRAIN] Optionally project the incremental update via CBF-QP
    ...
    # [CBF-QP TRAIN] If using CBF-QP, compute v_safe (projected) and integrate with delta_safe.
    if use_cbf:
        # Build A,b once at x_curr (detached), then project delta (both Euler and RK2 use same linearization)
        with torch.no_grad():
            A, b = projector.build_Ab(x_curr_gt.detach(), scene.detach())

    # Track delta_ref for distillation (check if projection actually changed anything)
    delta_ref_final = None
    delta_safe_final = None

    # Use RK2 (Heun) method if enabled, otherwise use Euler method
    if config.multi_pref_rollout_use_rk2:
        # RK2: x_{n+1} = x_n + Δλ * 0.5*(v0 + v1)
        if use_cbf:
            # Step 1: project Euler increment
            delta1_ref = dlambda * v_pred
            delta1_safe, _info1 = projector.project_delta(delta1_ref, A, b)
            x_euler = x_curr_gt + delta1_safe

            # Step 2: predict v1 at safe intermediate point
            v1 = model.predict_vec(scene, x_euler, lambda_next_norm, lambda_next_norm)

            delta2_ref = dlambda * 0.5 * (v_pred + v1)
            delta2_safe, _info2 = projector.project_delta(delta2_ref, A, b)

            x_pred = x_curr_gt + delta2_safe
            delta_ref_final = delta2_ref
            delta_safe_final = delta2_safe
        else:
            v1 = model.predict_vec(scene, x_curr_gt + dlambda * v_pred, lambda_next_norm, lambda_next_norm)
            x_pred = x_curr_gt + dlambda * 0.5 * (v_pred + v1)
    else:
        # Euler method: single-step integration
        if use_cbf:
            delta_ref = dlambda * v_pred
            delta_safe, _info = projector.project_delta(delta_ref, A, b)
            x_pred = x_curr_gt + delta_safe
            delta_ref_final = delta_ref
            delta_safe_final = delta_safe
        else:
            x_pred = x_curr_gt + dlambda * v_pred

    # ========== "Two-Leg" Loss Design ==========
    
    # Leg 1: Main supervision - align raw velocity with target (preserves learning objective)
    loss_v_raw = torch.mean((v_pred - v_target) ** 2)
    
    # Leg 2: Endpoint loss - uses projected trajectory (ensures actual execution quality)
    ...
    # Leg 3: Distillation loss - encourage v_pred ≈ v_used (reduce projection at inference)
    loss_distill = torch.tensor(0.0, device=device)
    
    if use_cbf and projector.cfg.distill_weight > 0 and delta_ref_final is not None:
        # Get distillation config
        distill_warmup = int(getattr(config, 'multi_pref_cbf_distill_warmup_epoch', 200))
        distill_eps = float(getattr(config, 'multi_pref_cbf_distill_eps', 1e-4))
        
        # Only apply distillation after warmup epochs (let model learn basic mapping first)
        if epoch >= distill_warmup:
            # Only apply if projection actually changed something (I = ||delta_safe - delta_ref|| > eps)
            with torch.no_grad():
                proj_change = torch.norm(delta_safe_final - delta_ref_final, dim=-1)  # [B]
                proj_mask = (proj_change > distill_eps).float()  # [B]
            
            # Only compute distillation where projection was triggered
            if proj_mask.sum() > 0:
                v_used = delta_safe_final / (dlambda + 1e-12)
                # Use detached teacher (v_used.detach()) to avoid gradient competition
                distill_per_sample = torch.mean((v_pred - v_used.detach()) ** 2, dim=-1)  # [B]
                loss_distill = torch.mean(distill_per_sample * proj_mask)
    
    # ========== Total Loss ==========
    gamma = float(getattr(projector, 'cfg', None) and projector.cfg.distill_weight or 0.0) if use_cbf else 0.0
    
    total_loss = alpha * loss_v_raw + beta * loss_endpoint + gamma * loss_distill
