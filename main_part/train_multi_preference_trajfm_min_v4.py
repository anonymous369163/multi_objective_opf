...
Supports: simple, vae, rectified, traj_rectified, diffusion  # [MOD]
...
def _train_step(model, batch_x, batch_idx, y_train_by_pref, lambda_values, lc_max,
    ...
    elif model_type == 'traj_rectified':  # [NEW] trajectory-level rectified flow matching
        # ------------------------------------------------------------
        # Build a full front trajectory for each sample in the batch:
        #   Y_star: [B, K, D] stacked over ALL preferences (lambda_values order)
        #   Y0:     [B, K, D] coarse trajectory from pretrain_model (or Gaussian)
        # ------------------------------------------------------------
        K = len(lambda_values)
        # Preference grid normalized to [0,1] (shared across batch)
        pref_grid = torch.tensor([lc / lc_max for lc in lambda_values], device=device, dtype=torch.float32)  # [K]

        # Stack GT trajectory
        y_list = []
        for lc in lambda_values:
            # y_train_by_pref[lc]: [N, D] on device
            y_list.append(y_train_by_pref[lc][batch_idx])  # [B, D]
        Y_star = torch.stack(y_list, dim=1)  # [B, K, D]

        # Build coarse trajectory Y0
        if pretrain_model is not None:
            with torch.no_grad(): 
                x_rep = batch_x[:, None, :].expand(B, K, batch_x.shape[1]).reshape(B * K, batch_x.shape[1])  # [B*K, C]
                pref_rep = pref_grid[None, :, None].expand(B, K, 1).reshape(B * K, 1)  # [B*K, 1]
                if hasattr(pretrain_model, 'pref_dim'):
                    y0_rep = pretrain_model(x_rep, use_mean=True, pref=pref_rep)
                else:
                    y0_rep = pretrain_model(torch.cat([x_rep, pref_rep], dim=1), use_mean=True)
                Y0 = y0_rep.view(B, K, -1)
        else:
            Y0 = torch.randn_like(Y_star)

        # Rectified trajectory flow path
        t_batch = torch.rand([B, 1], device=device)
        Yt, V_target = model.flow_forward_traj(Y_star, t_batch, Y0)   # [B,K,D], [B,K,D]
        V_pred = model(batch_x, Yt, t_batch, pref_grid)               # [B,K,D]

        loss_fm = model.loss(V_pred, V_target)

        # Optional: endpoint consistency (one-step estimator to Y*)
        # Y_hat ≈ Yt + (1-t)*V_pred   (since V is constant along rectified path)
        t_expand = t_batch.view(B, 1, 1)
        Y_hat = Yt + (1.0 - t_expand) * V_pred
        loss_end = ((Y_hat - Y_star) ** 2).mean()  # [MOD] avoid F.mse_loss

        # Optional: smoothness along preference axis (encourage a coherent front)
        # (Use on Y_hat to regularize the predicted front)
        diff = Y_hat[:, 1:, :] - Y_hat[:, :-1, :]
        loss_smooth = (diff ** 2).mean()

        w_end = float(getattr(config, 'traj_end_weight', float(os.environ.get('TRAJ_END_W', '0.1'))))
        w_smooth = float(getattr(config, 'traj_smooth_weight', float(os.environ.get('TRAJ_SMOOTH_W', '0.01'))))
        return loss_fm + w_end * loss_end + w_smooth * loss_smooth
...
def create_model(config, input_dim, output_dim, pref_dim, Vscale, Vbias):
    ...
    from net_utiles import FM, VAE, DM, TrajectoryFM  # [MOD] trajectory-level FM
    ...
    if model_type == 'simple':
        ...
        model = TrajectoryFM(
            ...
            hidden_dim=int(os.environ.get('TRAJ_HIDDEN_DIM', str(max(128, config.hidden_dim)))),
            num_conv_layers=int(os.environ.get('TRAJ_CONV_LAYERS', '4')),
            kernel_size=int(os.environ.get('TRAJ_KERNEL', '3')),
            output_norm=False,
        )
